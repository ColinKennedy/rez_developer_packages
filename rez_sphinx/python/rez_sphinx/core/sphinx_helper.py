...
def _scan_for_configuration_path(directory):
    ...
    Args:
        directory (str):
            A path on-disk to search within for the `Sphinx conf.py`_.

    Raises:
        RuntimeError: If no `Sphinx conf.py`_ could be found.

    Returns:
        str: The path on-disk to the `Sphinx conf.py`_.
